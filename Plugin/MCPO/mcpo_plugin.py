...
# Defensive imports for optional third-party libraries. Plugins may run inside
# Docker containers where not all deps are installed; report missing deps
# instead of crashing.
try:
...
class MCPOPlugin:
    ...
    def _load_config(self) -> Dict[str, Any]:
        """加载配置"""
        # 加载插件特定的config.env文件（如果 python-dotenv 可用）
        plugin_env_path = os.path.join(os.path.dirname(__file__), "config.env")
        if _HAS_DOTENV and os.path.exists(plugin_env_path):
            try:
                load_dotenv(plugin_env_path, override=True)
            except Exception as e:
                # 解析失败时记录但继续，后续会使用环境变量的默认值
                if hasattr(self, "logger"):
                    self.logger.warning(f"Failed to parse plugin config.env: {e}")
        else:
            if not _HAS_DOTENV and hasattr(self, "logger"):
                self.logger.info(
                    "python-dotenv not installed; skipping parsing of config.env"
                )
        ...
        if hasattr(self, "logger"):
            ...
            self.logger.info(
                f"Environment DEFAULT_TIMEZONE: {os.getenv('DEFAULT_TIMEZONE', 'UTC')}"
            )  # 诊断日志
        ...
        self.logger.info(
            f"Loaded config: Port={config['MCPO_PORT']}, ConfigPath={config['MCP_CONFIG_PATH']}, HotReload={config['MCPO_HOT_RELOAD']}, CustomConfigName={config['MCPO_CONFIG_NAME']}"
        )
    ...
    def _find_mcpo_process(self) -> Optional[int]:
        ...
        try:
            ...
            if self.mcpo_pid_file.exists():
                try:
                    ...
                    try:
                        ...
                        if (
                            process.is_running()
                            and "mcpo" in " ".join(process.cmdline()).lower()
                        ):
                            return pid
            ...
            if not _HAS_PSUTIL:
                # psutil 不可用，无法通过进程表查找
                self.logger.warning("psutil not installed: skipping process table scan")
                return None
            ...
            for proc in psutil.process_iter(["pid", "name", "cmdline"]):
                try:
                    ...
                    if (
                        "mcpo" in cmdline_str
                        and str(self.config["MCPO_PORT"]) in cmdline_str
                    ):
                        # 双重验证：检查端口绑定
                        try:
                            ...
                            for conn in connections:
                                if (
                                    conn.laddr.port == self.config["MCPO_PORT"]
                                    and conn.status == "LISTEN"
                                ):
                                    self.logger.info(
                                        f"Found MCPO process by port: PID {proc.pid}"
                                    )
                                    return proc.pid
                        except (psutil.AccessDenied, psutil.NoSuchProcess):
                            # 如果无法访问连接信息，只要命令行匹配就返回
                            self.logger.info(
                                f"Found MCPO process by cmdline: PID {proc.pid}"
                            )
                            return proc.pid
    ...
    def _is_server_running(self) -> bool:
        ...
        try:
            if not _HAS_REQUESTS:
                # requests 不可用，无法通过 HTTP 检查
                if hasattr(self, "logger"):
                    self.logger.warning(
                        "requests not installed: cannot perform HTTP health check"
                    )
                return False
    ...
    def _start_mcpo_server(self) -> bool:
        ...
        try:
            ...
            cmd = [
                ...
                config_path,
                "--port",
                str(self.config["MCPO_PORT"]),
                "--api-key",
                self.config["MCPO_API_KEY"],
            ]
            ...
            # --- [调试补丁 v1.1] ---
            # 重定向 stdout 和 stderr 到日志文件，而不是 DEVNULL
            plugin_dir = os.path.dirname(os.path.abspath(__file__))
            stdout_log_path = os.path.join(plugin_dir, "mcpo_stdout.log")
            stderr_log_path = os.path.join(plugin_dir, "mcpo_stderr.log")
            self.logger.info(f"Redirecting mcpo stdout to: {stdout_log_path}")
            self.logger.info(f"Redirecting mcpo stderr to: {stderr_log_path}")
            ...
            self.mcpo_process = subprocess.Popen(
                ...
                stdout=stdout_log,  # <-- [调试补丁] 修改
                stderr=stderr_log,  # <-- [调试补丁] 修改
                ...
                try:
                    ...
                    if process.is_running():
                        ...
                        try:
                            ...
                            # 强制终止
                            self.logger.warning(
                                "Graceful termination timeout, forcing kill"
                            )
                            process.kill()
                            try:
        ...
        example_config = {
            "mcpServers": {
                "time": {
                    ...
                    "args": [
                        "mcp-server-time",
                        f"--local-timezone={self.config['DEFAULT_TIMEZONE']}",
                    ],
                }
            }
        }
    ...
    def list_tools(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 解析描述中的服务器链接，支持相对路径 (/time/docs) 和绝对 URL (http://host/time/docs)
            import re
            from urllib.parse import urlparse

            # 支持两类链接格式： [name](/time/docs) 或 [name](http://host/time/docs)
            ...
            # 备用解析：如果描述中未找到链接，尝试从 openapi 的 servers 字段提取路径
            if not servers:
                try:
                    main_servers = main_spec.get("servers", [])
            ...
            for server in servers:
                try:
                    ...
                    self.logger.warning(
                        f"Failed to get tools from server {server}: {server_error}"
                    )
                    continue
        ...
        if request_body:
            ...
            if "$ref" in schema:
                ...
                if resolved_schema:
                    ...
                    parameters["note"] = (
                        "Parameters defined by schema reference (unable to resolve)"
                    )
            else:
                # 直接定义的参数
    ...
    def _resolve_schema_ref(
        self, ref_path: str, post_info: Dict[str, Any]
        ...
        try:
            ...
            if ref_path.startswith("#/components/schemas/"):
                ...
                for server in [
                    ...
                    try:
                        server_spec = self._make_request(
                            "GET", f"/{server}/openapi.json"
                        )
    ...
    def get_tool_info(self, tool_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if tool_name in tools["tools"]:
                return {"success": True, "tool_info": tools["tools"][tool_name]}
            else:
        ...
        try:
            if operation == "start":
                ...
                if self._is_server_running():
                    ...
                    if pid:
                        self.logger.info(
                            f"MCPO server is already running with PID: {pid}"
                        )
                        return {
                            "success": True,
                        ...
                        self.logger.info(
                            "MCPO server is running but PID not found, service is healthy"
                        )
                        return {
                            "success": True,
                        ...
                        try:
                            ...
                            try:
                                ...
                                if not process.is_running():
                                    self.logger.info(
                                        f"Cleaning up stale PID file for non-existent process {old_pid}"
                                    )
                                    self.mcpo_pid_file.unlink(missing_ok=True)
                            except (psutil.NoSuchProcess, psutil.AccessDenied):
                                self.logger.info(
                    ...
                    return {
                        ...
                        "message": (
                            "MCPO server started"
                            if success
                            else "Failed to start MCPO server"
                        ),
                    }
                ...
                if not self._is_server_running():
                    ...
                    return {
                        ...
                        "message": (
                            "MCPO server stopped"
                            if success
                            else "Failed to stop MCPO server"
                        ),
                    }
                ...
                if is_running:
                    self.logger.info(
                        "Service is running, performing restart (stop + start)"
                    )
                    # 服务正在运行，执行完整的重启流程
                    stop_success = self._stop_mcpo_server()
                    if not stop_success:
                        self.logger.warning(
                            "Stop operation had issues, but proceeding with start"
                        )
                    time.sleep(2)  # 等待完全停止
                    ...
                    return {
                        "success": success,
                        "message": (
                            f"MCPO server restarted (was running)"
                            if success
                            else "Failed to restart MCPO server (was running)"
                        ),
                    }
                else:
                    self.logger.info(
                        "Service not running, restart becomes direct start"
                    )
                    # 服务未运行，重启等同于直接启动
                    # 清理可能存在的 PID 文件
                    if self.mcpo_pid_file.exists():
                    ...
                    return {
                        "success": success,
                        "message": (
                            f"MCPO server started (was stopped)"
                            if success
                            else "Failed to start MCPO server (was stopped)"
                        ),
                    }
    ...
    def discover_tools(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "tools_count": (
                    tools_result.get("count", 0) if tools_result["success"] else 0
                ),
            }
    ...
    def list_configs(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                if os.path.exists(custom_config_dir) and os.path.isdir(
                    custom_config_dir
                ):
                            ...
                            configs.append(
                                {
                                    ...
                                    "config_name": config_name,
                                    "exists": True,
                                    "current": self.config["MCP_CONFIG_PATH"]
                                    == file_path,
                                }
                            )
            except Exception as e:
...
def main():
    ...
    try:
        ...
        try:
            ...
            print(
                json.dumps(
                    {"status": "error", "error": f"Invalid JSON input: {input_data}"}
                )
            )
            sys.exit(1)
        ...
        if (
            ...
            and plugin.config["MCPO_AUTO_START"]
            and not plugin._is_server_running()
        ):
        ...
        print(
            json.dumps(
                {"status": "error", "error": f"Plugin execution failed: {str(e)}"}
            )
        )
        sys.exit(1)
